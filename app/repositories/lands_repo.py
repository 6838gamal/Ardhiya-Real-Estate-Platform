...
class LandsRepository:
    ...
    # ============ القوائم ============
    def list_published(self, *, land_type=None, purpose=None, city=None,
                       limit: int = 60, offset: int = 0) -> Sequence[Land]:
        stmt = select(Land).where(Land.is_published.is_(True))
        if land_type: stmt = stmt.where(Land.land_type == land_type)
        if purpose:   stmt = stmt.where(Land.purpose   == purpose)
        if city:      stmt = stmt.where(Land.city      == city)
        stmt = (stmt.order_by(Land.featured.desc(), Land.created_at.desc())
                    .limit(limit).offset(offset))
        return self.db.execute(stmt).scalars().all()

    def count_published(self, *, land_type=None, purpose=None, city=None) -> int:
    ...
    def list_cities(self) -> list[str]:
        stmt = (select(distinct(Land.city))
